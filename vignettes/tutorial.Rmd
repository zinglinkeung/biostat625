...
all(c(all.equal(lm_fit$residuals, linreg_fit$residuals),
      ...
      all.equal(summary_lm$coefficients, linreg_fit$coefficients), 
...
From the results above, `linreg` with qr method is `r round(itr.ration[1], digits = 2)` times faster than `stats::lm`, `linreg` with svd method is `r round(itr.ration[2], digits = 2)` times faster than `stats::lm`,`linreg` use basic least square calculation is `r round(itr.ration[3], digits = 2)` times faster than `stats::lm`.
